...
* @date 2020/03/12	(eus)
* @brief Test octree data structure
*
...
* @param mesh_data
*/
	void load_meshes(MeshData mesh_data[Demo_octree::MeshType::mesh_type_count]) {
		load_mesh_obj("../resources/meshes/triangle.obj", nullptr, &mesh_data[Demo_octree::mesh_triangle]);
		load_mesh_obj("../resources/meshes/sphere.obj", nullptr, &mesh_data[Demo_octree::mesh_sphere]);
		load_mesh_obj("../resources/meshes/segment.obj", nullptr, &mesh_data[Demo_octree::mesh_segment]);
		load_mesh_obj("../resources/meshes/quad.obj", nullptr, &mesh_data[Demo_octree::mesh_quad]);
		load_mesh_obj("../resources/meshes/octohedron.obj", nullptr, &mesh_data[Demo_octree::mesh_octohedron]);
		load_mesh_obj("../resources/meshes/icosahedron.obj", nullptr, &mesh_data[Demo_octree::mesh_icosahedron]);
		load_mesh_obj("../resources/meshes/gourd.obj", nullptr, &mesh_data[Demo_octree::mesh_gourd]);
		load_mesh_obj("../resources/meshes/cylinder.obj", nullptr, &mesh_data[Demo_octree::mesh_cylinder]);
		load_mesh_obj("../resources/meshes/cube.obj", nullptr, &mesh_data[Demo_octree::mesh_cube]);
		load_mesh_obj("../resources/meshes/bunny.obj", nullptr, &mesh_data[Demo_octree::mesh_bunny]);
	}
	/**
*
* @param MIN
...
*/
	Object add_random_object(int MIN, int MAX, int index, MeshData* mesh_data)
	{
		...
		int rand_mesh_type = glm::linearRand(0, (int)Demo_octree::mesh_type_count - 1);
...
*/
...
#if 0
	TEST(bv_octree, create_random_objects_once)
	{
		// LOAD DATA
	...
	TEST(bv_octree, objects_contained_100)
	{
		...
		for (auto &o : objects)
			octree.add_object(o);
		// check containment
		auto is_contained = [&octree](const Octree::node& n) {
			// do not check root
			if (&n == &octree.root())
				return;
			// every object in node contained
			for (auto o : n.objects) {
				intersection_type result = intersection_aabb_aabb(o->get_aabb(), n.get_aabb());
				EXPECT_EQ(result, INSIDE);
			}
		};

		octree.traverse_preorder(is_contained);
	}
	TEST(bv_octree, objects_contained_1000)
	{
	...
	TEST(bv_octree, good_node_codes)
	{
		auto objects = read_objects("../tests/bounding_volume_hierarchy/random_objects_100");
		// create octree
		Octree octree;
		...
		// check containment
		auto is_contained = [&octree](const Octree::node& n) {
			Octree::node* found_node = octree.get_node(n.code);
			//if (found_node == nullptr)
			//	assert(0);
			EXPECT_EQ(&n, found_node);
		};
